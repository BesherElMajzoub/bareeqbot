...
type Row = {
    ...
    plan_price?: { duration_months: number; price: string; currency: string; plan?: { name: string } } | null;
};
    ...
    return (
        <>
            ...
            <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 p-4">
                <h1 className="text-2xl font-semibold tracking-tight">{t('admin.subscription_requests')}</h1>
                        ...
                        <table className="w-full text-start text-sm">
                            <thead className="text-muted-foreground">
                                <tr className="border-b border-sidebar-border/70">
                                    <th className="p-3 text-start">{t('admin.tenant')}</th>
                            ...
                            <tbody>
                                {requests.data.map((row) => (
                                    <tr key={row.id} className="border-b border-sidebar-border/40 last:border-0">
                                        <td className="p-3">{row.tenant?.name ?? '—'}</td>
                                        <td className="p-3">{row.plan_price?.plan?.name ?? '—'}</td>
                                        <td className="p-3">{row.plan_price?.duration_months ?? '—'}</td>
                                        <td className="p-3">
...
AdminSubscriptionRequests.layout = {
    breadcrumbs: [{ title: 'طلبات الاشتراك', href: subscriptionRequests.index().url }],
};
